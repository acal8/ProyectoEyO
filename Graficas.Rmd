...
for (i in seq_along(gamma_values)) {
  ...
  Umv_rel <- getUEval(alpha_hat, mu_hat, se_hat, Xtrain, Xtest, current_gammaMV, passChecks, Umv)
...
for (i in seq_along(gamma_values)) {
  ...
  # b. Evaluación con restricción POSITIVA
  passChecks <- getChecks(alpha_hat, mode=c("sum1","pos"))
  ret <- getRet(alpha_hat, Xtest, passChecks)
  Umv_rel <- getUEval(alpha_hat, mu_hat, se_hat, Xtrain, Xtest, current_gammaMVPos, passChecks, Umv)
