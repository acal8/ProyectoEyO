...
kable(df_log[,-4], 
      caption = "Retorno y utilidad acumulada para cada gamma",
      digits = c(2, 4, 2))
```


El comportamiento que se observa es extremo y atípico en comparación con una curva teórica suave. Obtenemos valores extremadamente grandes que caen rápidamente a 0. La función de utilidad agregada, sin embargo, sí que presenta un comportamiento más relativamente más suave.

Para valores $\gamma$ inferiores a 8, tenemos una zona de inestabilidad en la que la utilidad agregada es extremadamente volátil Obtenemos, por tanto, un portafolio muy sensible y posiblemente inestable.
...
Con todo esto, encontrar un valor óptimo de $\gamma$ en este caso no es tan sencillo. Asumir demasiado riesgo puede llevarnos a grandes pérdidas. Lo que parece claro es que deberíamos elegir un $\gamma \geq 8$, e incluso $\gamma \geq 15$, si lo que priorizamos es la seguridad en nuestra inversión.

# Resultados

Visualicemos los resultados obtenidos a 24 meses vista en la siguiente tabla.
